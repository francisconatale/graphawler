...
afterAll(async () => {
  await logger.save();
  const { generateHtmlReport } = await import('../../src/reporter/html_generator');
  await generateHtmlReport(logger.getResult(), './output/test-visual_report.html');
  await browser.close();
});
...
test('test-visual', async () => {
  ...
  await page.waitForTimeout(1500);
  lastStepId = await recordStep('loading', page.url(), lastStepId, '¿Olvidaste tu contraseña?', 'a.text-xs.font-bold.text-primary.hidden', 1);

  // --- Click: Volver al login ---
  await page.click('a.inline-flex.items-center.gap-2.text-on-surface-variant.text-sm.font-bold.transition-colors');
  await page.waitForTimeout(1500);
  lastStepId = await recordStep('loading', page.url(), lastStepId, 'Volver al login', 'a.inline-flex.items-center.gap-2.text-on-surface-variant.text-sm.font-bold.transition-colors', 1);

  // --- Click: Registrate acá ---
  await page.click('a.text-sm.font-bold.text-primary');
  await page.waitForTimeout(1500);
  lastStepId = await recordStep('loading', page.url(), lastStepId, 'Registrate acá', 'a.text-sm.font-bold.text-primary', 1);

  // --- Click: Registrate acá ---
  await page.click('a.text-sm.font-bold.text-primary');
  await page.waitForTimeout(1500);
  lastStepId = await recordStep('modal', page.url(), lastStepId, 'Registrate acá', 'a.text-sm.font-bold.text-primary', 1);
